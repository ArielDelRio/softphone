...
| Outbound call (mute / hold / transfer) | Inbound call (IVR → CS group) |
| --- | --- |
| ![Outbound call preview](assets/preview-outbound.gif) | ![Inbound call preview](assets/preview-inbound.gif) |
...
```bash
npm install @arieldrio/softphone
```

## Peer Dependencies

This package expects these to be installed in the consuming app:
...
Proprietary / `UNLICENSED` — see `LICENSE`. Redistribution and modification are not permitted without explicit written permission.

## Requirements and Dependencies

- [Node.js v18+](https://nodejs.org/en/download/) - Required for building and running the demo (Vite 5 requires Node 18+).
...
```jsx
import Softphone from "@arieldrio/softphone";
...
- **`id`** _`(default: generateUUID())`_: Other unique identifier for the contact.
...
```jsx
import { useSoftphone } from "@arieldrio/softphone";
...
```jsx
import { useSideBar } from "@arieldrio/softphone";
...
```typescript
import { utils } from "@arieldrio/softphone";
...
```jsx
import { LookupInput } from "@arieldrio/softphone";
...
```jsx
import { ActionButton } from "@arieldrio/softphone";
...
```jsx
import { ContactUI } from "@arieldrio/softphone";
