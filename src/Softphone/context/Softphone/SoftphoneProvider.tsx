...
import TwilioErrorsHandler from "./TwilioErrorsHandler";
...
const twilioErrorsHandler = new TwilioErrorsHandler();
...
export const SoftphoneProvider = ({
  ...
  const addDeviceListeners = (device: Device, events: Events) => {
    ...
    device.on("registered", () => {
      // events?.onRegisteredChange?.(true, getEventContext());
    });

    device.on("registering", () => {});

    device.on("unregistered", () => {
      // events?.onRegisteredChange?.(false, getEventContext());
    });
  ...
  const addCallListeners = (call: Call, events: Events) => {
    ...
    call.on("error", (twilioError: TwilioError.TwilioError) => {
      ...
      const handledError = twilioErrorsHandler.handle(twilioError);
      setAlert(handledError);
    });
